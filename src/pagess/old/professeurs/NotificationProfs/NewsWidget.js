...
const Newss = ({ classes }) => {
  ...
  useEffect(function () {
    ...
    axios
      ...
      .get(`http://www.pointofsaleseedigitalaency.xyz/public/APIUser/Notficationparents/${d}`)
      .then(res => {
        seCoursM(res.data)
    ...
    <ul className={classes.newsList}>
      {CoursM.map((m,) => (
        <li className={classes.listRow} key={m.title}>
          <div className={classes.iconBlock}>
          <span className={`${classes.icon}`} style={{ backgroundColor: '#ffb703',marginRight:'15px', marginTop:'10px' }}>
