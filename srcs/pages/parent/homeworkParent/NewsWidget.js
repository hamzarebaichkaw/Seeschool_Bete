...
const Newss = ({ classes,data }) => {
  ...
  useEffect(function () {
    ...
    axios
      ...
      .get(`http://www.pointofsaleseedigitalaency.xyz/public/APIUser/Notfication/${d}`)
      .then(res => {
        seCoursM(res.data)
      ...
      {data.map((m,) => (
        <li className={classes.listRow} key={m.title}>
          ...
          <div className={classes.newsmInfo}>
            <Typography variant="h3"><a className={classes.newsHeader} href="#/app/grid"> {m.Matiere} titre ici </a></Typography>
            <div>
              {m.Matiere} description de travail icisssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss
            </div>
            <time className={classes.timeBlock}>{m.type} deadline</time>
          </div>
