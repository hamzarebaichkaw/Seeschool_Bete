  ...
  useEffect(function () {
    ...
    axios
      ...
      .then(res => {
        seCoursM(res.data )
        res.data.map((e)=> {
          ph = ph + ( parseInt(e.["Controle 1"]) +parseInt( e.Partique)  +(2 * parseInt(e.["Controle 2"]) ) ) / 4
          

        } )
        setmay((ph / res.data.length).toFixed(2))
        if (ph / res.data.length < 10) {setmayColor("#dc3545")}
        else if (ph / res.data.length < 15) {setmayColor("#ffc107")}
